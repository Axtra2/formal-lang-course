...
def cfpq_with_tensor(
    ...
    n_rsm_states = sum([len(box.dfa.states) for box in rsm.boxes.values()])
    ...
    for N, box in rsm.boxes.items():
        for from_state, transitions in box.dfa.to_dict().items():
            for symbol, to_state in transitions.items():
                ...
                rsm_mat.setdefault(
                    symbol.value, dok_matrix((n_rsm_states, n_rsm_states), dtype=bool)
        ...
        for symbol, to_states in transitions.items():
            ...
            for to_state in to_states:
                ...
                graph_mat.setdefault(
                    symbol.value,
                    dok_matrix((n_graph_states, n_graph_states), dtype=bool),
                )[from_idx, to_idx] = True
    ...
    while True:
        ...
        for from_idx, to_idx in zip(*m.nonzero()):
            ...
            if from_rsm_state in rsm_start_states and to_rsm_state in rsm_final_states:
                ...
                graph_mat.setdefault(
                    N, dok_matrix((n_graph_states, n_graph_states), dtype=bool)
    ...
    for from_graph_state, to_graph_state in product(start_nodes, final_nodes):
        from_graph_idx = graph_state_to_idx[from_graph_state]
        to_graph_idx = graph_state_to_idx[to_graph_state]
